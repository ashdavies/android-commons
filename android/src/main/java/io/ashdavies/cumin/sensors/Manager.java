...
import io.ashdavies.cumin.rx.RxSupport;
...
public class Manager {
    ...
    public Observable<Event> observe(final int type, final int period) {
        if (!hasSensor(type)) {
            throw new RuntimeException(String.format(Locale.getDefault(), "Sensor id %d not available", type));
        }
        ...
        return Observable.create(new Observable.OnSubscribe<Event>() {
            @Override public void call(final Subscriber<? super Event> subscriber) {
                ...
                subscriber.add(RxSupport.scheduleUnsubscription(new Action0() {
